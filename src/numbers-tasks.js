...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const angleBetweenVectors = Math.acos(numerator / denominator);
...
function roundToPowerOfTen(num, pow) {
  const terminator = 10 ** pow;
  const result = Math.round(num / terminator) * terminator;
  return result;
}
...
function toNumber(value, def) {
  const valueToNumber = Number.isNaN(Number(value)) ? def : Number(value);
...
function getFibonacciNumber(index) {
  ...
  for (let i = 0; i < index - 1; i += 1) {
    fibonachiArr.push(
      fibonachiArr[fibonachiArr.length - 1] +
        fibonachiArr[fibonachiArr.length - 2]
    );
  }

  return fibonachiArr[index];
}
...
function isPowerOfTwo(num) {
  const powerTwo = Number.isInteger(Math.log2(num));
  return powerTwo;
}
...
function numberToStringInBase(number, base) {
  const radixNum = number.toString(base);
  return radixNum;
}
...
function toPrecision(number, precision) {
  const representation = number.toPrecision(precision);
...
function getIntegerOnString(str, base) {
  const binaryRepresentation = Number.parseInt(str, base);
...
function getSumOfNumbers(x1, x2, x3) {
  const sumOfNum = toFixed(x1 + x2 + x3, 1);
  return sumOfNum;
}
...
function getRandomInteger(min, max) {
  const randomaiser = Math.floor(Math.random() * (max - min + 1)) + min;
  return randomaiser;
}
